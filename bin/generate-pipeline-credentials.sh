...
if [ $(contains_string ${is_vault_installed} "command not found") == "true" ] ; then
...
if [ "${has_pcf}" == "Y" ] ; then
    echo -e "Does your ${yellow_color}cloud-native project${no_color} share a previously set up ${yellow_color}PCF API endpoint, domain, organization, and space${no_color}?"
    echo -e "Enter ${cyan_color}'Y'${no_color} for Yes and ${cyan_color}'N'${no_color} for No or leave blank, followed by [ENTER]:"
    read has_pcf_api_domain_org_space
    echo ""
    ...
    if [ "${has_pcf_api_domain_org_space}" == "N" ] ; then
        ...
        read pcf_domain_name
        echo ""

        if [ "${pcf_domain_name}" == "" ] ; then
            pcf_domain_name=${default_pcf_domain_name}
        fi

        pcf_domain_name=$(to_lower_case ${pcf_domain_name})
        pcf_domain_name=$(remove_special_chars_but_period ${pcf_domain_name})
...
if [ "${github_shared_pipeline_email}" != "" ] ; then
    ...
    shared_pipeline_git_repo_private_key=$(generate_github_repo_ssh_keys \
        ${ssh_dir} \
...
if [ "${github_shared_pipeline_email}" != "" ] ; then
    ...
    github_deploy_key_id=$(get_github_repo_deploy_key_id \
        ${github_api_uri} \
        ${github_user} \
        ${github_token} \
        ${github_org} \
